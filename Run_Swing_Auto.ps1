...
foreach ($lib in $libs) {
    ...
    if (!(Test-Path $fullPath)) {
        ...
        # Try to find any version of this jar in the repo
        # Extract artifact id approximately (this is hacky but helps)
        $parts = $lib -split "\\"
        if ($parts.Count -gt 3) {
...
if ($LASTEXITCODE -eq 0) {
    ...
    Start-Process -FilePath $java -ArgumentList "-cp `"$classpath`" com.example.MainApp"
} else {
